...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      ...
      let { data: recent_documents, error: document_error } = await supabase
      .from('recent_documents')
      .select('name, document_type, status, recipients, due_date, public_url, user_id')
      .or(`user_id.eq.${user.id},recipients.cs.{${user.email}}`)
                      ...
                      recentDocuments.map((doc, index) => (
                        ...
                        >
                          <td className="px-6 py-4 whitespace-nowrap">
                            ...
                            <Link to={`/editor?url=${doc.public_url}&name=${doc.name}`} className="flex items-center">
                              {doc.user_id==user.id?<File className="w-5 h-5 text-red-500 mr-2" />:<File className="w-5 h-5 text-yellow-500 mr-2" />}
